...
We are using rgb formated CSS variables to define our light & dark mode [theme colors](/docs/guide/theming). Why we are not using hex code directly? Because opacity modifiers won't work according to the [TailwindCSS documentation](https://tailwindcss.com/docs/customizing-colors#using-css-variables). Please copy and paste this code in to your app style file.
