---
sidebar_position: 25
---

import { Pagination } from 'rizzui';
import Paper from '@site/src/components/paper';
import {
	ChevronDoubleRightIcon,
	ChevronDoubleLeftIcon,
} from '@heroicons/react/24/outline';

# Pagination

We used `rc-pagination` package to build pagination component. See their [official documentation](https://pagination-react-component.vercel.app/) for more info.

```tsx
import { Pagination } from 'rizzui';
```

<div className="mb-28" />

## Default

The default style of **Pagination**.

<Paper>
	<Pagination total={25} defaultCurrent={1} className="[&_li]:!mt-0" />
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return <Pagination total={25} defaultCurrent={1} />;
}
```

<div className="mb-16" />

## Variants

You can change the style of **Pagination** with property **variant**.

<Paper className="flex-row flex-wrap justify-between">
	<Pagination
		total={25}
		variant="solid"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		variant="flat"
		defaultCurrent={1}
		className="[&_li]:!mt-0 [&_li>a]:!text-gray-1000"
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return (
		<>
			<Pagination total={25} variant="solid" defaultCurrent={1} />
			<Pagination total={25} variant="flat" defaultCurrent={1} />
		</>
	);
}
```

<div className="mb-16" />

## Rounded

You can change the style of **Pagination** with property [**rounded**](#pagination-rounded).

<Paper className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-8">
	<Pagination
		total={25}
		rounded="sm"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination total={25} defaultCurrent={1} className="[&_li]:!mt-0" />
	<Pagination
		total={25}
		rounded="lg"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		rounded="none"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		rounded="full"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return (
		<>
			<Pagination total={25} rounded="sm" defaultCurrent={1} />
			<Pagination total={25} variant="solid" defaultCurrent={1} />
			<Pagination total={25} rounded="lg" defaultCurrent={1} />
			<Pagination total={25} rounded="none" defaultCurrent={1} />
			<Pagination total={25} rounded="full" defaultCurrent={1} />
		</>
	);
}
```

<div className="mb-16" />

## Colors

You can change the style of **Pagination** with property [**color**](#pagination-colors).

<Paper className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-8">
	<Pagination total={25} defaultCurrent={1} className="[&_li]:!mt-0" />
	<Pagination
		total={25}
		color="primary"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		color="secondary"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		color="danger"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		color="info"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		color="success"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
	<Pagination
		total={25}
		color="warning"
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return (
		<>
			<Pagination total={25} defaultCurrent={1} />
			<Pagination total={25} color="primary" defaultCurrent={1} />
			<Pagination total={25} color="secondary" defaultCurrent={1} />
			<Pagination total={25} color="danger" defaultCurrent={1} />
			<Pagination total={25} color="info" defaultCurrent={1} />
			<Pagination total={25} color="success" defaultCurrent={1} />
			<Pagination total={25} color="warning" defaultCurrent={1} />
		</>
	);
}
```

<div className="mb-16" />

## Disabled

You can change the style of **Pagination** with property **disabled**.

<Paper>
	<Pagination
		total={25}
		defaultCurrent={1}
		className="[&_li]:!mt-0 [&_li>a]:!text-[#00000040]"
		disabled
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return <Pagination total={25} defaultCurrent={1} disabled />;
}
```

<div className="mb-16" />

## With Outline

You can change the style of **Pagination** with property **outline**.

<Paper>
	<Pagination
		total={25}
		outline={true}
		defaultCurrent={1}
		className="[&_li]:!mt-0"
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return <Pagination total={25} outline={true} defaultCurrent={1} />;
}
```

<div className="mb-16" />

## With Custom Previous & Next

You can change the style of **Pagination** with property **prevIcon** & **nextIcon**.

<Paper>
	<Pagination
		total={100}
		defaultCurrent={5}
		nextIcon="Next"
		prevIcon="Previous"
		className="[&_li]:!mt-0"
		prevIconClassName="py-0 text-gray-500 !leading-[26px]"
		nextIconClassName="py-0 text-gray-500 !leading-[26px]"
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';

export default function App() {
	return (
		<Pagination
			total={100}
			defaultCurrent={5}
			nextIcon="Next"
			prevIcon="Previous"
			prevIconClassName="py-0 text-gray-500 !leading-[26px]"
			nextIconClassName="py-0 text-gray-500 !leading-[26px]"
		/>
	);
}
```

<div className="mb-16" />

## With Custom Prev Next Jump

You can change the style of **Pagination** with property **jumpPrevIcon** & **jumpNextIcon**.

<Paper>
	<Pagination
		outline
		total={100}
		defaultCurrent={5}
		className="[&_li]:!mt-0"
		jumpPrevIcon={<ChevronDoubleLeftIcon className="h-4 w-4" />}
		jumpNextIcon={<ChevronDoubleRightIcon className="h-4 w-4" />}
	/>
</Paper>

```tsx
import { Pagination } from 'rizzui';
import {
	ChevronDoubleRightIcon,
	ChevronDoubleLeftIcon,
} from '@heroicons/react/24/outline';

export default function App() {
	return (
		<Pagination
			outline
			total={100}
			defaultCurrent={5}
			jumpPrevIcon={<ChevronDoubleLeftIcon className="h-4 w-4" />}
			jumpNextIcon={<ChevronDoubleRightIcon className="h-4 w-4" />}
		/>
	);
}
```

<div className="mb-20" />

## APIs

<hr />

<div className="mb-10" />

### Pagination Props

Here is the API documentation of the Pagination component.

|         Props         |                      Type                       |            Description            |   Default   |
| :-------------------: | :---------------------------------------------: | :-------------------------------: | :---------: |
|    defaultCurrent     |                    `number`                     |                                   |    \_ \_    |
|         total         |                    `number`                     |                                   |    \_ \_    |
|        variant        |                 `solid` `flat`                  | These are the variants we support |  `"solid"`  |
|        rounded        |    [PaginationRounded](#pagination-rounded)     |      Rounded variations are       | `"DEFAULT"` |
|         color         |     [PaginationColors](#pagination-colors)      |        Change Loader color        | `"DEFAULT"` |
|        outline        |                    `boolean`                    |    Whether show outline or not    |   `false`   |
|       disabled        |                    `boolean`                    |                                   |    \_ \_    |
|   prevIconClassName   |                    `string`                     | Pass className for previous icon  |    \_ \_    |
|   nextIconClassName   |                    `string`                     |   Pass className for next icon    |    \_ \_    |
| jumpPrevIconClassName |                    `string`                     | Pass className for previous icon  |    \_ \_    |
| jumpNextIconClassName |                    `string`                     |   Pass className for next icon    |    \_ \_    |
|       onChange        |  `((page: number, pageSize: number) => void)`   |                                   |    \_ \_    |
|   onShowSizeChange    |   `((current: number, size: number) => void)`   |                                   |    \_ \_    |
|      itemRender       | [PaginationItemRender](#pagination-item-render) |                                   |    \_ \_    |
|       showTotal       |  [PaginationShowTotal](#pagination-show-total)  |                                   |    \_ \_    |
|       className       |                    `string`                     |                                   |    \_ \_    |
|    selectPrefixCls    |                    `string`                     |                                   |    \_ \_    |
|       prefixCls       |                    `string`                     |                                   |    \_ \_    |
|    pageSizeOptions    |              `string[]` `number[]`              |                                   |    \_ \_    |
|        current        |                    `number`                     |                                   |    \_ \_    |
|       pageSize        |                    `number`                     |                                   |    \_ \_    |
|    defaultPageSize    |                    `number`                     |                                   |    \_ \_    |
|   hideOnSinglePage    |                    `boolean`                    |                                   |    \_ \_    |
|    showSizeChanger    |                    `boolean`                    |                                   |    \_ \_    |
|     showLessItems     |                    `boolean`                    |                                   |    \_ \_    |
|  showPrevNextJumpers  |                    `boolean`                    |                                   |    \_ \_    |
|    showQuickJumper    |               `boolean` `object`                |                                   |    \_ \_    |
|       showTitle       |                    `boolean`                    |                                   |    \_ \_    |
|        simple         |                    `boolean`                    |                                   |    \_ \_    |
|        locale         |               `PaginationLocale`                |                                   |    \_ \_    |
|         style         |                 `CSSProperties`                 |                                   |    \_ \_    |
| selectComponentClass  |               `ComponentType<{}>`               |                                   |    \_ \_    |
|       prevIcon        |         `ReactNode` `ComponentType<{}>`         |                                   |    \_ \_    |
|       nextIcon        |         `ReactNode` `ComponentType<{}>`         |                                   |    \_ \_    |
|     jumpPrevIcon      |         `ReactNode` `ComponentType<{}>`         |                                   |    \_ \_    |
|     jumpNextIcon      |         `ReactNode` `ComponentType<{}>`         |                                   |    \_ \_    |

<div className="mb-10" />

### Pagination Rounded

```ts
type PaginationRounded = 'sm' | 'DEFAULT' | 'lg' | 'none' | 'full';
```

<div className="mb-10" />

### Pagination Colors

```ts
type PaginationColors =
	| 'DEFAULT'
	| 'primary'
	| 'secondary'
	| 'danger'
	| 'info'
	| 'success'
	| 'warning';
```

<div className="mb-10" />

### Pagination Item Render

```ts
type PaginationItemRender = (
	page: number,
	type: 'page' | 'next' | 'prev' | 'jump-prev' | 'jump-next',
	element: ReactNode
) => ReactNode;
```

<div className="mb-10" />

### Pagination Show Total

```ts
type PaginationShowTotal = (
	total: number,
	range: [number, number]
) => ReactNode;
```
