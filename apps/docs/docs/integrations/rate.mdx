    ...
    <div className={cn('rizzui-rate', className)}>
      ...
      <RcRate
        ...
        character={({ index }: RcStarProps) => (
          <div className={cn(characterClass(), characterClassName)}>
...
<Paper className="gap-4 items-start">
  ...
  <Rate label="Medium (Default)" size="md" className="[&_li]:!mt-0 [&_ul]:!pl-0" />
...
Add helpful tooltips to guide users through each rating level:
...
| **tooltips** | `string[]` | Array of tooltip texts for each rating level | `undefined` |
...
- **Provide tooltips** - Use tooltips to explain each rating level for better UX
