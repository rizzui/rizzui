...
const fileUploader = tv({
  slots: {
    container:
      'flex items-center peer w-full transition duration-200 rounded-[var(--border-radius)] border-[length:var(--border-width)]',
  ...
  variants: {
    variant: {
      ...
      flat: {
        container:
          'bg-muted/70 backdrop-blur border-border focus-within:ring-[0.8px] ring-border hover:border-primary focus-within:border-primary focus-within:ring-primary',
      },
      text: {
        container:
...
export interface FileUploaderProps
  ...
  variant?: VariantProps<typeof fileUploader>['variant'];
  size?: VariantProps<typeof fileUploader>['size'];
  placeholder?: string;
  disabled?: boolean;
...
export function FileUploader({
  ...
  return (
    <div
      className={cn('rizzui-file-uploader-root', 'flex flex-col', className)}
    >
      <label className="block">
        {label ? (
          <span
            className={cn(
              'rizzui-file-uploader-label',
              'block',
      ...
      {!error && helperText ? (
        <FieldHelperText
          size={size}
          className={cn(
            'rizzui-file-uploader-helper-text',
            disabled && 'text-muted-foreground',
            helperClassName
          )}
        >
      ...
      {error ? (
        <FieldErrorText
          size={size}
          error={error}
          className={cn('rizzui-file-uploader-error-text', errorClassName)}
        />
      ) : null}
    </div>
  );
