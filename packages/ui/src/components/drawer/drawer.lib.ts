...
export function useResizeHandler({ placement, customSize = 384 }: ResizeProps) {
  ...
  let containerInitialSize = isPlacementOnYAxis(placement) ? containerRef.current?.clientHeight : containerRef.current?.clientWidth;
  const [width, setWidth] = useState<number>(containerInitialSize ?? customSize);
  ...
  const handleMouseDown = (e: MouseEvent<HTMLDivElement>) => {
    ...
    setStartPosition(() => isPlacementOnYAxis(placement) ? e.clientY : e.clientX);
    setIsResizing(() => true);
  };
